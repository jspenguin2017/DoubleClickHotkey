...
`Debug` and `RelWithDebInfo` debugging information intact. Use C stdio with explicit flushing for console output and
integer conversion for error codes to avoid C++ stream and locale machinery; preserve output behavior.
...
3. Verify ASLR/NX remain enabled. When changing build settings, also verify that `Debug` and `RelWithDebInfo` retain
...
Manually verify affected Windows behavior: F13 handling, console output/visibility, and launch commands. Report
unavailable checks as pending; cross-compilation and binary inspection do not exercise runtime behavior.
